% Process Systems Engineering - Conceptual Design of Hydrodealkylation of Toluene 
% Authors: Matteo Robbiano & Mohammad Sina Ghanbari Pakdehi
% Instructor: Prof. Davide Manca - Ing. Andrea Isella
% Politecnico di Milano - Spring 2024
...
Nout = [RVh(:,end) RVm(:,end) Tt(:,end) Bb(:,end) Dd(:,end)];
for i=1:length(Tvalues)
    no=Nin(i,:);     
    Tin=Tvalues(i);
    Toutsolution=fsolve(@(Tout)DeltaT_ad(Tout,no,Tin),Tin);
...
% 2st Scenario [M€/y] - BURN BIPHENYL
...
IC_stabilizer = M_and_S/280*4.7.*D_stabilizer.^1.55.*H_tot_stabilizer.*Fc_stabilizer/1e6; % [M€] IC of the stabilizer at various T.
...
for i=1:length(T_S)
    ...
    IC_product(i,:) = M_and_S/280*4.7.*D_product(i,:).^1.55.*H_tot_product(i,:).*Fc_product(i)/1e6; % [M€] IC of the product column at various T.
end

for i=1:length(Tvalues)
        IC_best_tray_product(i) = min(IC_product(:,i));
        
        [row,col] = find(IC_product==IC_best_tray_product(i));              % Finding the position of the lowest I.C for each T.
        
        Index_tray_sizing_prod(i) = row;                                    % Finding the tray sizing corresponding to the lowest I.C.
        
        Best_tray_prod(i) = T_S(Index_tray_sizing_prod(i));                 % [ft] Vector with the best tray sizing for the product column at various T.
end
...
for i=1:length(T_S)
    ...
    IC_recycle(i,:) = M_and_S/280*4.7.*D_recycle(i,:).^1.55.*H_tot_recycle(i,:).*Fc_recycle(i)/1e6; % [M€] IC of the recycle column at various T.
end

for i=1:length(Tvalues)
        IC_best_tray_recycle(i) = min(IC_recycle(:,i));
        
        [row,col] = find(IC_recycle==IC_best_tray_recycle(i));              % Finding the position of the lowest I.C for each T.
        
        Index_tray_sizing_rec(i) = row;                                     % Finding the tray sizing corresponding to the lowest I.C.
        
        Best_tray_rec(i) = T_S(Index_tray_sizing_rec(i));                   % [ft] Vector with the best tray sizing for the recycle column at various T.
end
...
A_cond = [A_cond_stab; A_cond_prod; A_cond_rec];                            % [ft2] heat exchange area of all the condensers in the separation section at various T.
IC_cond_tot = IC_cond_stab+IC_cond_prod+IC_cond_rec;                        % [M€] IC of all the condensers in the separation section at various T.
OPEX_cond_tot = OPEX_cond_stab+OPEX_cond_prod+OPEX_cond_rec;                % [M€/y] OC of all the condensers in the separation section at various T.
...
Fc_reb_stab = (Fd_fixed+Fp)*Fm_CS_SS;
...
Fc_reb_prod = (Fd_fixed+Fp)*Fm_CS_CS;
...
Fc_reb_rec = (Fd_fixed+Fp)*Fm_CS_CS;
...
A_reb = [A_reb_stab; A_reb_prod; A_reb_rec];                                % [ft2] heat exchange area of all the reboilers in the separation section at various T.
IC_reb_tot = IC_reb_stab+IC_reb_prod+IC_reb_rec;                            % [M€] IC of all the reboilers in the separation section at various T.
OPEX_reb_tot = OPEX_reb_stab+OPEX_reb_prod+OPEX_reb_rec;                    % [M€/y] OC of all the reboilers in the separation section at various T.
...
xlabel('Conversion (%)');ylabel('Selectivity (%)'); grid on
yline(96)
...
xlabel('Temperature [°C]'); ylabel('Conversion (%)'); grid on
str = {'© Matteo Robbiano &','Mohammad Sina Ghanbari Pakdehi'};
text(0.2,0.4,str,'FontSize',8)
...
xlabel('Temperature [°C]'); ylabel('Reactor volume [m^{3}]'); grid on
str = {'© Matteo Robbiano &','Mohammad Sina Ghanbari Pakdehi'};
text(0.2,0.4,str,'FontSize',8)
...
xlabel('Split Factor'); ylabel('Mole Fraction of H_{2} [-]'); grid on
str = {'© Matteo Robbiano &','Mohammad Sina Ghanbari Pakdehi'};
...
xlabel('Mole Fraction of H_{2} [-]'); ylabel('EP2 (M€/y)'); ylim([0 +Inf]); grid on
...
xlabel('Split Factor [-]'); ylabel('EP2 (M€/y)'); ylim([0 +Inf]); grid on
legend('600°C (Sell)','650°C (Sell)','700°C (Sell)','750°C (Sell)',...
    '600°C (Burn)','650°C (Burn)','700°C (Burn)','750°C (Burn)',...
    'location','northwest','NumColumns',2,'location','northeast')
str = {'© Matteo Robbiano &','Mohammad Sina Ghanbari Pakdehi'};
...
yline(0)
xlabel('Temperature [°C]'); ylabel('EP2 (M€/y)'); grid on
...
yline(0)
xlabel('Conversion (%)'); ylabel('EP2 (M€/y)'); grid on
...
hold off
xlabel('Split Factor [-]'); ylabel('Reactor Volume [m^{3}]'); grid on
str = {'© Matteo Robbiano &','Mohammad Sina Ghanbari Pakdehi'};
...
xlabel('Split Factor [-]'); ylabel('Reactor Diameter [m]'); grid on
str = {'© Matteo Robbiano &','Mohammad Sina Ghanbari Pakdehi'};
...
xlabel('Split Factor [-]'); ylabel('IC reactor [M€]'); grid on
str = {'© Matteo Robbiano &','Mohammad Sina Ghanbari Pakdehi'};
...
xlabel('Split Factor [-]'); ylabel('Recycle stream [kmol/h]'); grid on
legend('600°C','650°C','700°C','750°C')
str = {'© Matteo Robbiano &','Mohammad Sina Ghanbari Pakdehi'};
...
xlabel('Split factor [-]'); ylabel('I.C. of the compressor [M€]'); grid on
str = {'© Matteo Robbiano &','Mohammad Sina Ghanbari Pakdehi'};
...
xlabel('Split factor [-]'); ylabel('O.C. of the compressor [M€/y]'); grid on
legend('600°C','650°C','700°C','750°C')
hold off

% EP3 (both "sell" and "burn" scenarios) vs. Split factor
figure(20)
plot(SFvalues,EP3_sell(1,:),':',color=[0 0.4470 0.7410])
hold on
plot(SFvalues,EP3_sell(2,:),':',color=[0.8500 0.3250 0.0980])
plot(SFvalues,EP3_sell(3,:),':',color=[0.9290 0.6940 0.1250])
plot(SFvalues,EP3_sell(4,:),':',color=[0.4940 0.1840 0.5560])

plot(SFvalues,EP3_sell(1,:),color=[0 0.4470 0.7410])
plot(SFvalues,EP3_sell(2,:),color=[0.8500 0.3250 0.0980])
plot(SFvalues,EP3_sell(3,:),color=[0.9290 0.6940 0.1250])
plot(SFvalues,EP3_sell(4,:),color=[0.4940 0.1840 0.5560])
xlabel('Split Factor [-]'); ylabel('EP3 [M€/y]'); ylim([0 5]);grid on
...
xlabel('Temperature [°C]'); ylabel('Separation section cost [M€/y]');grid on
str = {'© Matteo Robbiano &','Mohammad Sina Ghanbari Pakdehi'};
...
xlabel('Temperature [°C]'); ylabel('EP4 [M€/y]'); grid on
str = {'© Matteo Robbiano &','Mohammad Sina Ghanbari Pakdehi'};
...
xlabel('Temperature [°C]'); ylabel('Economic potential [M€/y]'); grid on
legend('EP4','EP2','EP3','location','east')
str = {'© Matteo Robbiano &','Mohammad Sina Ghanbari Pakdehi'};
